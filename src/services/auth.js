...
async function resetPassword(email, action = 'RESET') {
  ...
  // prettier-ignore
  const url = `${process.env.ADMIN_ORIGIN}/#/reset_password?email=${encodeURIComponent(email)}&action=${action.toLowerCase()}&token=${encodeURIComponent(passwordResetToken)}`
  // prettier-ignore
  const html = `
    <p>Aloha,</p>
